...
class CharacterAdapter : RecyclerView.Adapter<CharacterAdapter.CharacterViewHolder>() {
  ...
  override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): CharacterViewHolder {
    val view = LayoutInflater.from(parent.context).inflate(
      R.layout.row_character_viewholder,
      parent, false
    )
  ...
  override fun onBindViewHolder(holder: CharacterViewHolder, position: Int) {
    ...
    with(holder) {
      characterNameTextView.text = String.format(
        characterNameTextView.context.getString(R.string.name_placeholder),
        character.name
      )
      characterRoleTextView.text = String.format(
        characterRoleTextView.context.getString(
          R.string
            .role_placeholder
        ), character.role?.capitalize() ?: "Unknown"
      )
      orderOfThePhoenixTextView.text = String.format(
        orderOfThePhoenixTextView.context.getString(
          R.string
            .orderOfThePhoenix_placeholder
  ...
  inner class CharacterViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
    val characterNameTextView: TextView = itemView.findViewById(R.id.character_name_textview)
      //itemView.character_name_textview
